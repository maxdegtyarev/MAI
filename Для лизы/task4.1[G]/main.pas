...
type
  ...
  Tmain_form = class(TForm)
    ...
    procedure sdnf_textChange(Sender: TObject);
    procedure sknf_saveClick(Sender: TObject);
    procedure CountFromFile;
  private
...
procedure Tmain_form.sdnf_textChange(Sender: TObject);
begin
...
//procedure Tmain_form.CountFromFile;
//begin
//
//end;
...
begin
  ...
  //Открываем файл
    AssignFile(Fl, 'kurs.exe_temp.txt');
    AssignFile(Fot, 'sdnf.txt'); //there
        {$I-}
    reset(Fl);
        {$I+}
    if IORESULT <> 0 then
    ...
    for i:= 1 to s.Length do begin
      if (s[i] in usednums) then begin
        s[i]:= chr((Ord(s[i]) - 64 + 48));
      end;
    end;

    for i:= 1 to s.Length do begin
      if s[i] in usingnums then begin
        if s[i-1] = '/' then begin
          s[i-1] := ' ';
          s[i] := '0';
        end
        else if s[i] in usingnums then begin
          s[i]:= '1';
        end;
      end
      else if (s[i] = '*') then
    ...
    for i:= 1 to s.Length do begin
       if (s[i] = '1') or (s[i]='0') then write(fot,s[i])
...
begin
  if (flag=true) then main_form.close;
end;
