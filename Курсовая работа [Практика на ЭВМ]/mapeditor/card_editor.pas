...
var
  i: integer;
begin
  if (button = mbLeft) and (brushe <> 0) then
  begin
    if act = True then
    begin
      ADD();
...
var
  ...
  fout: file of mapobj;
begin
  //Подправил и разобрался, что так делать нельзя. Спасибо Андрею Кохановскому)
  if (map_open.Execute) then
  begin
  ...
  {$I+}
    if IOResult <> 0 then begin
       ShowMessage('Ошибка! Проблемы с файлом');
       exit;
    end;
    onform := 0;
    SetLength(objects, 0);
    ...
    begin
      ...
      if (objects[onform - 1].cel <> -368) or (objects[onform - 1].vector > 4) or (objects[onform - 1].vector < 1) then
      begin
        ShowMessage(
